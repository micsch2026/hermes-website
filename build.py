...
def build_page(content_path, out_path):
    ...
    warning = '<!-- ⚠️ AUTO-GENERATED by build.py — DO NOT EDIT MANUALLY!\n     Source: src/content/' + os.path.basename(content_path) + '\n     To change: edit the source, then run: cd /root/.hermes/site && python3 build.py -->\n'
    with open(out_path, 'w') as f:
        f.write(warning + html)
...
def main():
    ...
    # 0. Sync template assets (bot.css, bot-components.js)
    for tmpl_file in ['bot.css', 'bot-components.js']:
        src = os.path.join(TMPL_DIR, tmpl_file)
        dst = os.path.join(SITE_DIR, 'assets', tmpl_file)
    ...
    if os.path.exists(bot_pages_script):
        print('  Generating bot dashboard pages...')
        subprocess.run([sys.executable, bot_pages_script], check=False)
